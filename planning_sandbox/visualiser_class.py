...
class Visualizer:
    ...
    def draw_obstacles(self):
        for obstacle in self.obstacles:
            ...
            debug_text = self.debug_font.render(f"O{obstacle}", True, WHITE)
            self.screen.blit(debug_text, (obstacle[0] * CELL_SIZE, obstacle[1] * CELL_SIZE))
    ...
    def draw_goals(self):
        for i, goal in enumerate(self.goals):
            ...
            self.screen.blit(text, (x * CELL_SIZE + CELL_SIZE // 3, y * CELL_SIZE + CELL_SIZE // 3))
            debug_text = self.debug_font.render(f"G{(x, y)}", True, BLACK)
            self.screen.blit(debug_text, (x * CELL_SIZE, y * CELL_SIZE))
